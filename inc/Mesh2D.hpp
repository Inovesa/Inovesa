...
public:
		...
		_axis(axis)
    {
		...
		data_t** data = new data_t*[size<0>()];
		data_t** data_tmp = new data_t*[size<0>()];
		for (unsigned int i=0; i<size<0>(); i++) {
			data[i] = &(_data1D[i*size<1>()]);
			data_tmp[i] = &(_data1D_tmp[i*size<1>()]);
        }
		_data = data;
		_data_tmp = data_tmp;
	...
	{
		const std::vector<data_t> noforce(size<0>(),0);
		rotateAndKick(deltat,noforce);
	}
