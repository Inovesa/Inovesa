...
parser.add_argument('--colmin', type=float,default=10)
parser.add_argument('--colmax', type=float,default=5e7)
...
spectrogram = plt.pcolormesh(freqs_scaled,currents,data,shading='flat',norm=matplotlib.colors.LogNorm(vmin=args.colmin,vmax=args.colmax))
plt.yscale('log', subsy=np.arange(1, 10, 1)[1:])
plt.gca().yaxis.set_minor_formatter(FuncFormatter(minor_format))
plt.gca().yaxis.set_major_formatter(FuncFormatter(lambda x, i=None: x))
plt.tick_params('x',which='major', direction='out', labelleft='on', width=2,length=5)
...
plt.xlabel("Scaled Frequency / kHz")
plt.ylabel("Scaled Bunch Current / mA")
spectrogram = plt.pcolormesh(freqs_scaled,currents,data,shading='flat',norm=matplotlib.colors.LogNorm(vmin=args.colmin,vmax=args.colmax*10))
